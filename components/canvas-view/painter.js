...
const StackBlur = require('./stackblur.min.js')
export default class Painter {
  ...
  _drawAbsImage(view) {
    ...
    if (view.gauss) {
      const data = this.ctx.getImageData(0, 0, 750, 1334)
      const emptyData = StackBlur.imageDataRGB(data, 0, 0, 750, 1334, 160)
      this.ctx.putImageData(emptyData, 0, 0)
    }
    this.ctx.restore()
    this._doBorder(view, width, height)
  }
